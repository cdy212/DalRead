...
@Component
public class LangJP extends Language{
	...
	public LangJP(Integer uid) {
		super.setUID(uid);
//		iSearchDic = new SearchDicJp();		
		tbl_DIC = Constants.TBL_DIC_JP;
		tbl_DIC_BOOKMARK = Constants.TBL_DIC_JP_BOOKMARK;
		tbl_USER_DIC = Constants.TBL_USER_DIC_JP_PREFIX + "_" + this.uid;		
...
//		String mixed = "This is a Japanese newspaper headline: ラドクリフ、マラソン五輪代表に1万m出場にも含み";
		...
		for (char c : strOneChar.toCharArray()) {
		    if (japaneseUnicodeBlocks.contains(UnicodeBlock.of(c))) {
		    	logger.debug(c + " is a Japanese character");
		    	return true;
		    } else {
		    	logger.debug(c + " is not a Japanese character");
		    	return false;
		    }
		}
        return false;
